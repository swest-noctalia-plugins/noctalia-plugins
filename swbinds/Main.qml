...
Item {
    ...
    function cleanupProcesses() {
        if (niriGlobProcess.running)
            niriGlobProcess.running = false;
        if (niriReadProcess.running)
            niriReadProcess.running = false;
        if (hyprGlobProcess.running)
            hyprGlobProcess.running = false;
        if (hyprReadProcess.running)
            hyprReadProcess.running = false;
        if (fishReadProcess.running)
            fishReadProcess.running = false;
    ...
    function clearParsingData() {
        // Clears only compositor parsing state — fish state is managed by the fish functions
        filesToParse = [];
        parsedFiles = {};
        accumulatedLines = [];
        currentLines = [];
    ...
    // Fish alias parsing state
    property var compositorCategories: []
    property var fishLines: []
    property var fishAliases: []
            ...
            saveToDb([
                {
                    ...
                    "binds": [
                        {
                            "keys": compositorName.toUpperCase(),
                            "desc": unsupportedMsg.short
                        },
                        {
                            "keys": "INFO",
                            "desc": unsupportedMsg.detail
                        }
                    ]
                }
            ]);
    ...
    function finalizeNiriBinds() {
        var categoryOrder = ["Noctalia", "Applications", "Window Management", "Column Navigation", "Window Focus", "Workspace Navigation", "Workspace Management", "Move Columns", "Move Windows", "Column Management", "Column Width", "Window Size", "Screenshots", "Power", "System", "Animations"];
        ...
        isCurrentlyParsing = false;
        clearParsingData();
        compositorCategories = categories;
        parseFishAliasesPhase();
    }
            ...
            if (exitCode === 0 && root.currentLines.length > 0) {
                for (var i = 0; i < root.currentLines.length; i++) {
                    ...
                    if (sourceMatch) {
                        ...
                        if (commentIdx > 0)
                            sourcePath = sourcePath.substring(0, commentIdx).trim();
    ...
    function parseHyprlandConfig(text) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (line.includes("bind") && line.includes('#"')) {
                ...
                if (currentCategory) {
                    ...
                    if (parts.length >= 2) {
                        ...
                        var mods = [];
                        if (modPart.includes(modVarUpper) || modPart.includes("SUPER"))
                            mods.push("Super");

                        if (modPart.includes("SHIFT"))
                            mods.push("Shift");
                        if (modPart.includes("CTRL") || modPart.includes("CONTROL"))
                            mods.push("Ctrl");
                        if (modPart.includes("ALT"))
                            mods.push("Alt");
    ...
    function formatNiriAction(action) {
        ...
        if (action.indexOf("noctalia-shell") !== -1 && action.indexOf("ipc") !== -1) {
            ...
            var ipcMatch = action.match(/ipc\s+call\s+(\w+)\s+(\w+)/) || action.match(/"ipc"\s+"call"\s+"(\w+)"\s+"(\w+)"/);
            if (ipcMatch) {
                ...
                return ipcMatch[1].replace(/([A-Z])/g, ' $1').trim() + ": " + ipcMatch[2].replace(/([A-Z])/g, ' $1').trim();
            }
        }
        ...
        return action.replace(/-/g, ' ').replace(/\b\w/g, function (l) {
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                if (root.fishLines.length < 5000) {
                    root.fishLines.push(data);
                }
            }
        }

        onExited: (exitCode, exitStatus) => {
            if (exitCode === 0 && root.fishLines.length > 0) {
                root.parseFishFileContent(root.fishLines.join("\n"));
            }
            root.fishLines = [];
            root.finalizeSave();
        }
    ...
    function parseFishAliasesPhase() {
        var showFish = pluginApi?.pluginSettings?.showFishAliases ?? true;
        if (!showFish) {
            finalizeSave();
            return;
        }
        ...
        var homeDir = Quickshell.env("HOME") || "";
        var fishPath = pluginApi?.pluginSettings?.fishConfigPath || "~/.config/fish/config.fish";
        fishPath = fishPath.replace(/^~/, homeDir);

        fishAliases = [];
        fishLines = [];
        fishReadProcess.command = ["cat", fishPath];
        fishReadProcess.running = true;
    }

    function parseFishFileContent(text) {
        var lines = text.split('\n');
        var aliasList = [];
        var abbrList = [];
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            var aliasEqMatch = line.match(/^alias\s+([\w.@:+-]+)\s*=\s*(.+)$/);
            if (aliasEqMatch) {
                var aName = aliasEqMatch[1];
                var aVal = aliasEqMatch[2].trim();
            ...
            // alias name 'value'  alias name "value"  alias name value
            var aliasSpaceMatch = line.match(/^alias\s+([\w.@:+-]+)\s+(.+)$/);
            if (aliasSpaceMatch) {
                var asName = aliasSpaceMatch[1];
                var asVal = aliasSpaceMatch[2].trim();
                if ((asVal.startsWith("'") && asVal.endsWith("'")) || (asVal.startsWith('"') && asVal.endsWith('"'))) {
                    asVal = asVal.slice(1, -1);
                }
                aliasList.push({
                    "keys": asName,
                    "desc": asVal
            ...
            // ---- abbr ----
            // abbr -a name 'expansion'  abbr --add name "expansion"
            // abbr -a --position anywhere name expansion
            var abbrMatch = line.match(/^abbr\s+(?:-[-\w]+\s+)*([\w.@:+-]+)\s+(.+)$/);
            if (abbrMatch) {
                // Skip if the "name" looks like a flag (starts with -)
                if (abbrMatch[1].startsWith('-'))
                    continue;
                var bName = abbrMatch[1];
                var bVal = abbrMatch[2].trim();
                if ((bVal.startsWith("'") && bVal.endsWith("'")) || (bVal.startsWith('"') && bVal.endsWith('"'))) {
                    bVal = bVal.slice(1, -1);
                }
                abbrList.push({
                    "keys": bName,
        ...
        if (aliasList.length > 0) {
            fishAliases.push({
                "title": pluginApi?.tr("panel.fish-aliases") || "Fish Aliases",
                "binds": aliasList
            });
        }
        if (abbrList.length > 0) {
            fishAliases.push({
                "title": pluginApi?.tr("panel.fish-abbreviations") || "Fish Abbreviations",
                "binds": abbrList
            });
        }
    }
    ...
    function finalizeSave() {
        var allCategories = compositorCategories.slice();
        for (var f = 0; f < fishAliases.length; f++) {
            allCategories.push(fishAliases[f]);
        }
        fishAliases = [];
        fishLines = [];
